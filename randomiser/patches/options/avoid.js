...
const binary = require('../../../util/binary.js');

function apply(rom) {
    // Alter level checking functions
    binary.writeArray(rom, 0xCA054, [0x1, 0x20]);
    binary.writeArray(rom, 0xCB558, [0x13, 0x1C]);

    // Change Avoid to a toggle
    binary.writeArray(rom, 0xE1266, [0x13, 0x68, 0x53, 0x40]);

    // Jump to custom Avoid text display function
    binary.writeArray(rom, 0xE1354, [0x12, 0xF0, 0xD4, 0xFD, 0x0, 0xE0]);

    // Insert Avoid text display function
    binary.writeArray(rom, 0xF3F00, [0x60, 0xB5, 0x7, 0x4D, 0x2D, 0x68, 0x7, 0x48, 0x0, 0x2D, 0x5, 0xDC, 0x6, 0x0, 0x96, 0x20, 0x4, 0x21, 0x48, 0xF7,
