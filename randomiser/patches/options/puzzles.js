...
function applyGabomba(mapCode, sum) {
    mapCode[1649][0] = true;
    writeArray(mapCode[1649][1], 0x3606, [sum & 0xF, 0x20, 0x0, 0xBD]);
}

function applyGaiaRock(mapCode, sum) {
    mapCode[1665][0] = true;
    writeArray(mapCode[1665][1], 0x42AE, [0x1, 0x48, 0xB, 0xE0, 0x0, 0x0, sum & 0xFF, (sum >> 8) & 0xFF, 0x0, 0x0]);
}

function applyTrialRoad(mapCode, sum) {
    mapCode[1687][0] = true;
    writeArray(mapCode[1687][1], 0x2492, [0x1, 0x48, 0x7, 0xE0, 0x0, 0x0, sum & 0xFF, (sum >> 8) & 0xFF, 0x0, 0x0]);
}
