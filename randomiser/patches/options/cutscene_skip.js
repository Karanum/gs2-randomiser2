...
function apply(mapCode, text) {
    ...
    applyAlhafra(mapCode[1637], mapCode[1640]);
    ...
    applyNorthernReaches(mapCode[1703]);
    applyMarsLighthouse(mapCode[1707], mapCode[1710]);
    ...
    textutil.writeLine(text, 0x2D8C, "Fight Doom Dragon?\x1E");
...
function applyKandorean(mapCode) {
    ...
    writeArray(mapCode[1], 0x23E8, [0x20, 0xB5, 0x0, 0x20, 0x02, 0xF0, 0xC0, 0xFF, 0x80, 0x20, 0x80, 0x4, 0x5, 0x0, 0xCA, 0x21, 0x9, 
    ...
    writeArray(mapCode[1], 0x2336, [0x2, 0xB0, 0x0, 0xBD]);
...
function applyLemurianShip(intMapCode, extMapCode) {
    ...
    writeArray(intMapCode[1], 0x169C, [0x80, 0x20, 0x0, 0x1, 0xDE, 0x30, 0x1, 0xF0, 0x9D, 0xFA, 0xD1, 0xE0]);
...
function applyMadra(extMapCode, intMapCode) {
    ...
    writeArray(intMapCode[1], 0x102E, [0xD, 0xE0]);
    writeArray(intMapCode[1], 0x1090, [0xB0, 0xE0]);
    writeArray(intMapCode[1], 0x1202, [0x2, 0xE0]);
    writeArray(intMapCode[1], 0x1218, [0x17, 0xE0]);
    writeArray(intMapCode[1], 0x1258, [0x10, 0xE0]);
    writeArray(intMapCode[1], 0x1286, [0x31, 0xE0]);
    intMapCode[1][0x1252] = 0x19;

    //ASM for skipping Mayor scenes
    writeArray(extMapCode[1], 0x19E8, [0x21, 0x20, 0x0, 0x21, 0x0, 0x22, 0x1, 0xF0, 0xCB, 0xF8, 0x1, 0xF0, 0x21, 0xF9, 0x8C, 0x22, 
        0x4, 0x20, 0x96, 0x21, 0x52, 0x0, 0x1, 0xF0, 0xB3, 0xF8, 0x20, 0x20, 0x0, 0x2, 0xCD, 0x30, 0x1, 0xF0, 0xE2, 0xF8, 0x4A, 0xE1]);
    writeArray(extMapCode[1], 0x1D2C, [0x2C, 0xE0]);
    writeArray(extMapCode[1], 0x1D88, [0x0, 0x48, 0x0, 0x47, 0x8D, 0xA9, 0x0, 0x2]);
...
function applyGaroh(mapCode) {
    ...
    writeArray(mapCode[1], 0x38A2, [0x52, 0x4]);
    writeArray(mapCode[1], 0x38B4, [0x3, 0xE0]);
    writeArray(mapCode[1], 0x38DC, [0x26, 0xE0]);
    writeArray(mapCode[1], 0x3938, [0x32, 0xE0]);
    writeArray(mapCode[1], 0x39B0, [0x20, 0x20, 0x0, 0x3, 0xCA, 0x30, 0x0, 0x2, 0xB3, 0x30, 0x0, 0x47]);
    writeArray(mapCode[1], 0x4B7A, [0x18, 0xE1]);
    writeArray(mapCode[1], 0x4DF6, [0x80, 0xE1]);
}
 ...
 * @param {MapCodeEntry} mapCode 
 */
function applyAlhafra(extMapCode, eastMapCode) {
    extMapCode[0] = true;
    eastMapCode[0] = true;

    //ASM for skipping the cutscenes with the hungry boy and his father
    writeArray(extMapCode[1], 0x33A, [0x9, 0xE0]);
    writeArray(extMapCode[1], 0x36C, [0x80, 0x20, 0x0, 0x1, 0xB6, 0x30, 0x1, 0xF0, 0x23, 0xFC, 0x1, 0xE0]);
    writeArray(extMapCode[1], 0x690, [0x80, 0x20, 0x0, 0x1, 0xB9, 0x30, 0x1, 0xF0, 0x91, 0xFA, 0x14, 0x20, 0x1, 0xF0, 0x9E, 0xFA, 0x1E, 0xE0]);
    writeArray(extMapCode[1], 0x74C, [0x60, 0xBD]);

    //ASM for skipping the mayor cutscene after Bursting the rock
    writeArray(eastMapCode[1], 0x205C, [0x90, 0x25, 0x2D, 0x1, 0x68, 0x1D, 0x2, 0xF0, 0xCF, 0xFC, 0xF, 0x35, 0x28, 0x1C, 0x2, 0xF0, 
        0xCB, 0xFC, 0x70, 0x35, 0x28, 0x1C, 0x2, 0xF0, 0xC7, 0xFC, 0x20, 0xBD]);
    writeArray(eastMapCode[1], 0x2A0A, [0xE0, 0xBD]);
}
...
function applyBriggs(mapCode) {
    ...
    writeArray(mapCode[1], 0x4F4, [0x0, 0xB5, 0x0, 0x20, 0x2, 0xF0, 0xA2, 0xFE, 0x4, 0x20, 0x0, 0x21, 0x0, 0x22, 0x0, 0x23, 0x2, 0xF0,
    ...
    writeArray(mapCode[1], 0x292, [0xF5, 0x0, 0xF5, 0x0]);

    // Updates function table
    writeArray(mapCode[1], 0x32A4, [0x41, 0x80, 0x3]);
}
...
function applyKibombo(mapCode) {
    ...
    writeArray(mapCode[1], 0x131A, [0x20, 0x4D, 0x99, 0x22, 0x12, 0x2, 0x28, 0x68, 0x1F, 0x49, 0x99, 0x32, 0x4, 0xF0, 0xD1, 0xFB, 0x99, 
    ...
    writeArray(mapCode[1], 0x4D04, [0xC0, 0x27, 0xBF, 0x4, 0xA, 0x20, 0x0, 0x21, 0x0, 0x22, 0x0, 0xF0, 0xFD, 0xFE, 0x0, 0xF0, 0x6B, 
...
function applyGabomba(mapCode, coreMapCode) {
    ...
    writeArray(mapCode[1], 0xFEC, [0x2, 0xE0]);

    // ASM for skipping the core opening cutscene
    writeArray(coreMapCode[1], 0x1766, [0xFF, 0xF7, 0xCB, 0xFB, 0xFF, 0xF7, 0x7B, 0xFC, 0x8, 0x20, 0x6A, 0x21, 0x98, 0x22, 0x89, 
    ...
    writeArray(coreMapCode[1], 0x194, [0x2, 0x4D, 0x3, 0x4E, 0x3, 0x4F, 0x0, 0x20, 0x80, 0x46, 0x48, 0xE3, 0x58, 0xA0, 0x0, 0x2, 
        0x54, 0x4, 0x0, 0x2, 0x40, 0x2, 0x0, 0x2]);
    writeArray(coreMapCode[1], 0x882, [0xD7, 0xE0]);

    // Updating the exit table to consolidate incorrect puzzle solutions and reroute the correct one
    writeArray(mapCode[1], 0x5FA4, [0x79, 0x30]);
    writeArray(mapCode[1], 0x5FA8, [0x79, 0x30]);
    writeArray(mapCode[1], 0x5FAC, [0x79, 0x30]);
    writeArray(mapCode[1], 0x5FB0, [0x78, 0x60]);

    // Updating the core's NPC table to remove Akafubu
    writeArray(coreMapCode[1], 0x2222, [0x0, 0x0]);
...
function applyGabombaCatacombs(mapCode) {
    ...
    writeArray(mapCode[1], 0xA02, [0x36, 0xE0]);
    writeArray(mapCode[1], 0xA96, [0x1, 0xE0]);
    writeArray(mapCode[1], 0xAF8, [0x24, 0xE1]);
    writeArray(mapCode[1], 0xD4E, [0xB, 0xE0]);
...
function applyAquaRock(apojiiMapCode, rockMapCode) {
    ...
    writeArray(apojiiMapCode[1], 0x9AA, [0x3E, 0xE0]);
    writeArray(apojiiMapCode[1], 0x2198, [0x9E, 0x50, 0xD0]);
    writeArray(rockMapCode[1], 0x16AA, [0x87, 0xE0]);
}
...
function applyEasternSeaIsles(mapCode) {
    ...
    writeArray(mapCode[1], 0x1E0C, [0x5C, 0xE0]);
    writeArray(mapCode[1], 0x1F02, [0x0, 0xE0]);
    writeArray(mapCode[1], 0x1F10, [0x0, 0xE0]);
    writeArray(mapCode[1], 0x1F28, [0x0, 0xE0]);

    // ASM for skipping the bird cutscene
    writeArray(mapCode[1], 0x5E0, [0x33, 0xE0]);
    writeArray(mapCode[1], 0x69C, [0x0, 0xE0]);

    // ASM for skipping the cow cutscene
    mapCode[1][0x729] = 0xD0;
    writeArray(mapCode[1], 0x72C, [0x14, 0xE0]);
    writeArray(mapCode[1], 0x760, [0x38, 0x68, 0x3, 0xF0, 0x8F, 0xFB, 0x80, 0x46, 0xA4, 0xE0]);
    writeArray(mapCode[1], 0x8F0, [0x15, 0xE0]);

    // ASM for skipping the dog cutscene
    writeArray(mapCode[1], 0xE74, [0x65, 0xE1]);
    writeArray(mapCode[1], 0x1150, [0x1B, 0xE0]);
    writeArray(mapCode[1], 0x1192, [0x0, 0xE0]);

    // ASM for skipping the turtle cutscenes
    writeArray(mapCode[1], 0x12AA, [0xC8, 0x20, 0xFF, 0x30, 0x2, 0xF0, 0xD1, 0xFD, 0xE7, 0xE0]);
    writeArray(mapCode[1], 0x14B6, [0x0, 0x1C]);
    writeArray(mapCode[1], 0x150A, [0x14, 0x20, 0x2, 0xF0, 0xA6, 0xFC, 0x8D, 0xE1]);
    writeArray(mapCode[1], 0x18BC, [0xA2, 0xE0]);
    writeArray(mapCode[1], 0x1A0C, [0x0, 0xE0]);
    writeArray(mapCode[1], 0x1AA2, [0x7A, 0xE0]);
    writeArray(mapCode[1], 0x1BA2, [0x0, 0xE0]);
    writeArray(mapCode[1], 0x2848, [0x9C, 0x21, 0xDC, 0x22, 0xC0, 0x23, 0x49, 0x4, 0x52, 0x4, 0x1B, 0x2, 0x1, 0xF0, 
        0x46, 0xFB, 0x1E, 0xE0]);
    writeArray(mapCode[1], 0x28B4, [0x6A, 0xE0]);
    writeArray(mapCode[1], 0x29D4, [0x4C, 0x21, 0x2E, 0x22, 0xC0, 0x23, 0x12, 0x1, 0x6, 0x32, 0x49, 0x4, 0x12, 0x4, 
        0x1B, 0x2, 0x1, 0xF0, 0x7E, 0xFA, 0x1D, 0xE0]);
    writeArray(mapCode[1], 0x2A4C, [0x70, 0xE0]);
}
...
function applyIzumo(mapCode) {
    ...
    writeArray(mapCode[1], 0x1E0A, [0x3A, 0xE0]);
    writeArray(mapCode[1], 0x1EF4, [0x50, 0xE0]);
    writeArray(mapCode[1], 0x1FBA, [0x14, 0xE0]);
}
...
function applyGaiaRock(extMapCode, intMapCode) {
    ...
    writeArray(extMapCode[1], 0x38C, [0xF, 0xE0]);
    writeArray(extMapCode[1], 0x8FA, [0x17, 0xE0]);

    // ASM for skipping the interior Susa cutscenes
    writeArray(intMapCode[1], 0x31A2, [0x6, 0xE0]);
    writeArray(intMapCode[1], 0x3770, [0xA, 0x20, 0x0, 0x21, 0x0, 0x22, 0x1, 0xF0, 0xA1, 0xFB, 0x1E, 0x23, 0x13, 0x22, 0x0, 
...
function applyChampa(mapCode) {
    ...
    writeArray(mapCode[1], 0x828, [0x8, 0x20, 0x0, 0x21, 0x5, 0x22, 0x2, 0xF0, 0x75, 0xFE, 0x0, 0x20, 0xFF, 0xF7, 0x22, 0xFF, 0x94, 0xE0]);
    writeArray(mapCode[1], 0x992, [0x8, 0x20, 0x0, 0x21, 0x5, 0x22, 0x2, 0xF0, 0xC0, 0xFD, 0x0, 0x20, 0xFF, 0xF7, 0x6D, 0xFE, 0x3D, 0xE0]);
    writeArray(mapCode[1], 0xAC0, [0x14, 0xE0]);
    writeArray(mapCode[1], 0xAFE, [0x4, 0x20, 0x20, 0x21, 0x9, 0x2, 0x0, 0x22, 0x2, 0xF0, 0x11, 0xFD, 0x8, 0x20, 0x40, 0x21, 0x9, 0x2, 
    ...
    writeArray(mapCode[1], 0xE90, [0x1, 0xE0]);
    writeArray(mapCode[1], 0xED6, [0x23, 0xE0]);

    // ASM for skipping the Avimander cutscenes
    writeArray(mapCode[1], 0x1188, [0xF1, 0x48, 0x5, 0x21, 0x2, 0xF0, 0xFE, 0xF9, 0x4, 0x20, 0x0, 0x21, 0x2, 0xF0, 0x6A, 0xF9, 0x0, 
        ...
        0x22, 0x3, 0x2]);
    writeArray(mapCode[1], 0x1896, [0xA4, 0x20, 0x1, 0x21, 0xF8, 0x22, 0x0, 0x23, 0x49, 0x42, 0x12, 0x4, 0x80, 0x4, 0x1, 0xF0, 0x5A, 
    ...
    // Updates function table
    writeArray(mapCode[1], 0x3590, [0x41, 0x80, 0x3]);
}
...
function applyContigo(mapCode) {
    ...
    writeArray(mapCode[1], 0x1D6A, [0x0, 0x20, 0x0, 0x21, 0x0, 0x22, 0x2, 0xF0, 0xB0, 0xF9, 0x1, 0x20, 0x0, 0x21, 0x0, 0x22, 0x2, 0xF0, 
...
function applyShamanVillage(mapCode, trMapCode) {
    ...
    writeArray(mapCode[1], 0x209C, [0x1A, 0xE0]);
    writeArray(mapCode[1], 0x20EC, [0x72, 0xE2]);
    writeArray(mapCode[1], 0x261C, [0xB4, 0x20, 0x2, 0xF0, 0x4D, 0xF9, 0xF, 0x20, 0x0, 0x21, 0x0, 0x22, 0x2, 0xF0, 0x84, 0xF9, 
    ...
    writeArray(mapCode[1], 0x30E6, [0x0, 0xF0, 0x1D, 0xFD, 0x1, 0xF0, 0xEF, 0xFB, 0xE0, 0xBD]);
    writeArray(mapCode[1], 0x4070, [0x3D, 0xD0]);
    ...
    writeArray(trMapCode[1], 0x17E2, [0x21, 0xE0]);
    writeArray(trMapCode[1], 0x184A, [0x0, 0x1C]);
    writeArray(trMapCode[1], 0x3576, [0x98, 0xE0]);
    writeArray(trMapCode[1], 0x36B8, [0x7D, 0xE0]);
...
function applyJupiterAerie(mapCode) {
    ...
    writeArray(mapCode[1], 0x65C, [0x20, 0xB5, 0xA0, 0x20, 0x0, 0x1, 0x23, 0x30, 0x5, 0xF0, 0x62, 0xF8, 0x0, 0x28, 0x55, 0xD0, 0x0, 
...
function applyLoho(mapCode) {
    ...
    writeArray(mapCode[1], 0x4D8, [0x10, 0x20, 0x0, 0x21, 0x0, 0x22, 0x0, 0xF0, 0x33, 0xFF, 0x11, 0x20, 0x0, 0x21, 0x0, 0x22, 0x0, 
...
/**
 * Applies cutscene skip to the Northern Reaches
 * @param {mapCodeEntry} mapCode 
 */
function applyNorthernReaches(mapCode) {
    mapCode[0] = true;

    // ASM for skipping the scene of firing the cannon at the ice wall
    writeArray(mapCode[1], 0x23C, [0x84, 0xE0]);
    writeArray(mapCode[1], 0x48A, [0x91, 0xE0]);
}

/**
 * Applies cutscene skip to Mars Lighthouse
 * @param {MapCodeEntry} basementMapCode 
 * @param {MapCodeEntry} aerieMapCode 
 */
function applyMarsLighthouse(basementMapCode, aerieMapCode) {
    basementMapCode[0] = true;
    aerieMapCode[0] = true;

    // ASM for skipping the Flame Dragons post-battle cutscene
    writeArray(basementMapCode[1], 0x8BC, [0xC9, 0xE0]);
    writeArray(basementMapCode[1], 0xA58, [0xA, 0xE0]);

    // ASM for skipping the Doom Dragon cutscene
    writeArray(aerieMapCode[1], 0x682, [0x2D, 0x20, 0x0, 0x2, 0x8C, 0x30, 0x5, 0x21, 0x4, 0xF0, 0xAB, 0xFF, 0x4, 0x20, 0x0, 0x21, 
        0x4, 0xF0, 0xCB, 0xFF, 0x0, 0x28, 0x8, 0xD0, 0x4, 0x20, 0x0, 0x21, 0x10, 0x22, 0x5, 0xF0, 0x80, 0xF8, 0x4, 0xF0, 0xB6, 0xFF, 
        0x4, 0x48, 0x0, 0x47, 0x3, 0x20, 0x0, 0x21, 0x5, 0xF0, 0x50, 0xF8, 0x2, 0x48, 0x2A, 0x1C, 0x37, 0x32, 0x0, 0x47, 0x81, 0x9E, 
        0x0, 0x2, 0x6D, 0x9E, 0x0, 0x2]);

    // Updating function table
    writeArray(aerieMapCode[1], 0x57A8, [0xF9, 0x85, 0xC]);
}
