...
function applyAlhafra(extMapCode, eastMapCode) {
    ...
    applyMapCode(extMapCode[1], 0x33A, [0x9, 0xE0]);
    applyMapCode(extMapCode[1], 0x36C, [0x80, 0x20, 0x0, 0x1, 0xB6, 0x30, 0x1, 0xF0, 0x23, 0xFC, 0x1, 0xE0]);
    applyMapCode(extMapCode[1], 0x690, [0x80, 0x20, 0x0, 0x1, 0xB9, 0x30, 0x1, 0xF0, 0x91, 0xFA, 0x14, 0x20, 0x1, 0xF0, 0x9E, 0xFA, 0x1E, 0xE0]);
    applyMapCode(extMapCode[1], 0x74C, [0x60, 0xBD]);

    //ASM for skipping the mayor cutscene after Bursting the rock
    applyMapCode(eastMapCode[1], 0x205C, [0x90, 0x25, 0x2D, 0x1, 0x68, 0x1D, 0x2, 0xF0, 0xCF, 0xFC, 0xF, 0x35, 0x28, 0x1C, 0x2, 0xF0, 
        0xCB, 0xFC, 0x70, 0x35, 0x28, 0x1C, 0x2, 0xF0, 0xC7, 0xFC, 0x20, 0xBD]);
    applyMapCode(eastMapCode[1], 0x2A0A, [0xE0, 0xBD]);
}
...
function applyEasternSeaIsles(mapCode) {
    ...
    applyMapCode(mapCode[1], 0x12AA, [0xC8, 0x20, 0xFF, 0x30, 0x2, 0xF0, 0xD1, 0xFD, 0xE7, 0xE0]);
...
function applyNorthernReaches(mapCode) {
    ...
    applyMapCode(mapCode[1], 0x23C, [0x84, 0xE0]);
    applyMapCode(mapCode[1], 0x48A, [0x91, 0xE0]);
}
...
function applyMarsLighthouse(basementMapCode, aerieMapCode) {
    ...
    applyMapCode(basementMapCode[1], 0x8BC, [0xC9, 0xE0]);
    applyMapCode(basementMapCode[1], 0xA58, [0xA, 0xE0]);

    // ASM for skipping the Doom Dragon cutscene
    applyMapCode(aerieMapCode[1], 0x682, [0x2D, 0x20, 0x0, 0x2, 0x8C, 0x30, 0x5, 0x21, 0x4, 0xF0, 0xAB, 0xFF, 0x4, 0x20, 0x0, 0x21, 
    ...
    applyMapCode(aerieMapCode[1], 0x57A8, [0xF9, 0x85, 0xC]);
}
